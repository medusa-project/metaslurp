module Admin
  ...
  class ContentServicesController < ControlPanelController
    ...
    def purge
      ...
      raise ActiveRecord::RecordNotFound unless @content_service
      begin
        DeleteContentServiceItemsJob.perform_later(@content_service)
      rescue => e
        flash['error'] = "#{e}"
      else
    ...
    def update
      begin
        @content_service.update!(sanitized_params)
    ...
    def sanitized_params
      params.require(:content_service).permit(:element_mappings,
                                              :harvest_type, :key, :name, :uri)
    end
